...
void serialInit(int arr[][5001]){
	int compval = 0;
	for(i = 1; i <= lenA; ++i){	
	    for(j = 1; j <= lenB; ++j){	
			if(FASTA1[i-1] == FASTA2[j-1]){				
				compval = (arr[i-1][j-1] + Match);	
			}
			if(compval < ((arr[i-1][j]) + Gap)){	    
				compval = ((arr[i-1][j]) + Gap);		
			}
			if(compval<(arr[i][j-1]+Gap)){	
			    compval=((arr[i][j-1])+Gap);   
			}
			if(compval < 0){
				compval = 0;
			}
			...
			if(FASTA1[i-1] != FASTA2[j-1]){					
				if(compval < (arr[i-1][j-1] + MissMatch)){	
					compval = arr[i-1][j-1] + MissMatch;
                }
				if(compval < ((arr[i-1][j]) + Gap)){	   
					compval = ((arr[i-1][j]) + Gap);	
				}
...
void parallelInit(int arr[][5001]){
...
id=omp_get_thread_omp();
Nthrds = omp_get_num_thread();
istart = id * N / Nthrds;
iend = (id + 1) * N / Nthrds;
*/